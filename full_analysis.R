...
plot <- ggplot(data = df, mapping = aes(x=X1,y=X2, colour = fit$cell_type)) + geom_point() + scale_colour_discrete(drop=TRUE, limits = levels(c("Pigmented","NCSC","Invasive","SMC"))) + labs(title = "Transcriptional Program Assignments", caption = "Learning rate given to Cellassign: 1e-3. No cycle removal.")
...
plot <- ggplot(data = df, mapping = aes(x=X1,y=X2, colour = fit$cell_type)) + geom_point() + scale_colour_discrete(drop=TRUE, limits = levels(c("Pigmented","NCSC","Invasive","SMC"))) + labs(title = "Transcriptional Program Assignments", caption = "Learning rate given to Cellassign: 1e-2. No cycle removal.")
...
plot <- ggplot(data = df, mapping = aes(x=X1,y=X2, colour = fit$cell_type)) + geom_point() + scale_colour_discrete(drop=TRUE, limits = levels(c("Pigmented","NCSC","Invasive","SMC", "Unassigned"))) + labs(title = "Transcriptional Program Assignments", caption = "Learning rate given to Cellassign: 1e-2. No cycle removal.")
